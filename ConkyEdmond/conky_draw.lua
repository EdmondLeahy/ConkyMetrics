require 'cairo'
package.path = package.path .. ';' .. os.getenv("HOME") .. '/.conky/ConkyEdmond/conky-draw/?.lua'
...
function draw_ring_graph_no_conky(display, element)
  -- draw a ring graph
  if element.max_value == 0 then
    error ("The value of max_value must be non-zero as it is used a divisor. Dividing by zero causes undefined behavior (usually it results in an 'inf').")
  end
  ...
  -- get current value
  local handle = io.popen(element.command)
  local value = tonumber(handle:read("*a")) - element.min_value

  if element.to_percent then
    value = (value*100) / element.percent_mod_maximum 
  end
...
requirements = {
    ...
    ring_graph_no_conky = {'center', 'radius', 'command'},
...
defaults = {
    ...
    ring_graph_no_conky = {
        ...
        draw_function = draw_ring_graph_no_conky,
    },
    line = {
        color = 0x00FF6E,
        alpha = 0.2,
